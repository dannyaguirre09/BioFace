...
class BarTop10PromedioIngresos extends React.Component {

    obtenerDatosContador = async () => {
        const uri = "https://localhost:44393/api/ReporteGeneral/?mejor=1";
        await fetch(uri)
            .then(res => res.json())
            .then(response => {
    ...
    render() {
        return (
            <div>
                ...
                <h4 className="text-center">Top 10 personas mejor promedio de hora de ingresos </h4>
                <Bar data={this.state} />
            </div>
        );
    }
...
export default withRouter(BarTop10PromedioIngresos)