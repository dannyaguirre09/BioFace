...
class ReconocimientoSalidaComponent extends React.Component {
    ...
    reconocimiento = async () => {
        await fetch('https://localhost:44393/api/ReconocimientoSalida', {
            method: 'POST',
            ...
            .then(data => {
                ...
                if (this.state.idPersona[0] === 1) {
                    this.apagarCamara()
                    this.props.history.push('/despedida/' + this.state.idPersona[1])
                } else {
                    this.apagarCamara()
...
export default withRouter(ReconocimientoSalidaComponent)