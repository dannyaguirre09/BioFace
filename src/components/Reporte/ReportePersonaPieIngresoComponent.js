...
class ReportePersonaPieIngresoComponent extends React.Component {

	obtenerDatosContador = async () => {
		const { id_persona } = this.props.match.params
		const uri = "https://localhost:44393/api/Reporte/?idPersonaConteoIngreso=" + id_persona
		await fetch(uri)
			.then(res => res.json())
			.then(response => {
	...
	cambiarDatos(datosNuevos) {
		...
		for (let i = 0; i < datosNuevos.length; i++) {
			...
			newlabels[i] = this.formatearFecha(datosNuevos[i].FechaIngreso);
	...
	render() {
		return (
			<div>
				...
				<h4 className="text-center">Número de Registros de Ingresos por Mes</h4>
				<Pie data={this.state} />
			</div>
...
export default withRouter(ReportePersonaPieIngresoComponent)